...
j=2; %实验次数计数初始值 
E=[];%开辟一个用于记录进行换股时换股价格的数组 0表示交易日为债券的的到期日,1表示提前回售，2表示在赎回终止日正常转股，其余数字表示最终正常转股价格
count=0; %用来记录在赎回条款日完成换股的次数
 ...
 for i=1:M %进行M次实验
    ...
    [exchangeprice,Exday] = Exchangeprice(19.69,20,10,price(i,:)); %计算出各段的转股价格和转股日期
      ...
      for k1=2:length(Exday) %检查k1-1段 %由于这段是后加的，为避免循环变量重名带来混乱，所以用了k1，后期将循环变量进行进行规范化
          [finalday,flag1] = Redemption(exchangeprice(k1-1),30,15,price(i,:),Exday(k1-1)+1,Exday(k1)-Exday(k1-1));%将日期修改下修当日，请关注？
    ...
    if (flag==1||flag==3) %如果在到期前转股，或者在赎回条款满足当天抛售 因为都是提前转股，只不过是被迫或者主动的区别，计算方法相同
       ...
       payoff=n*price(i,l)*(1-0.0005)*Restriction_discount(13.10,6,0.0028,0.143)+fix(l/245+113/365)*amount*r2; %计算最后的总收入（股票扣除印花税，不扣所得税）135为何要扣？
    ...
    if flag==2 %如果提前回售，则拿回3个利息加本息
        pv(i)=(amount*r2*2.75+amount)/(1+r1)^(l/245);%并非按3年计算，暂按2.75年修正，如代入l，数字好像错误，代入finalday，数字好像也是不对的，请核实该等数据合理性？
        E(i)=1;
    end
    flag=0; %将标志位复原
    j=2;
