...
flag=0;%用于记录是否下修
for i=checkday+46:checkday+Dayrange-1+46 %由于后面要计算i-20，所以先检查前20日的股价
...
if flag==0 %如果在第一段没有发生下修
for i=checkday+Dayrange+46:Length %检查后一段时间，始终保持Dayrange内的计数和价格总和（通过减除Dayrange前数据的方式实现）
    if price(i)<Exchangeprice*0.85
        count=count+1;
    end
